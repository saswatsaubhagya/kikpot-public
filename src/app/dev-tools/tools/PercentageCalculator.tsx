...
export default function PercentageCalculator() {
  ...
  const Stepper = ({ onDec, onInc }: { onDec: () => void; onInc: () => void }) => (
    <div className="flex items-center gap-2">
      <button type="button" className="shrink-0 px-3 h-10 rounded-xl border border-hairline text-text" onClick={onDec}>−</button>
      <button type="button" className="shrink-0 px-3 h-10 rounded-xl border border-hairline text-text" onClick={onInc}>+</button>
    </div>
  );
  ...
  const Result = ({ value, suffix }: { value: number; suffix?: string }) => (
    <div className="flex items-center gap-3">
      <div className="min-w-[7rem] h-10 px-3 rounded-2xl border border-hairline bg-surface text-text flex items-center">
        {Number.isFinite(value) ? (suffix ? `${value.toFixed(4)}${suffix}` : value.toFixed(4)) : "—"}
      </div>
      <button onClick={() => copy(value)} className="px-3 h-10 rounded-xl border border-hairline text-text hover:bg-surface-2" aria-label="Copy result">📋</button>
    </div>
  );
  ...
  return (
    <div className="space-y-6">
      ...
      <section className="p-5 rounded-2xl bg-surface border border-hairline space-y-3">
        <div className="text-dim text-sm">What is</div>
      ...
      <section className="p-5 rounded-2xl bg-surface border border-hairline space-y-3">
        <div className="grid grid-cols-1 md:grid-cols-[1fr_auto_auto_auto_1fr_auto_auto] gap-3 items-center">
          ...
          <div className="text-dim">is what percent of</div>
      ...
      {/* Percentage change */}
      <section className="p-5 rounded-2xl bg-surface border border-hairline space-y-3">
        <div className="text-dim text-sm">What is the percentage increase/decrease</div>
