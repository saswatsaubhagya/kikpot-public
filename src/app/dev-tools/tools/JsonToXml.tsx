...
export default function JsonToXml() {
  ...
  return (
    <div className="space-y-6">
      <p className="text-sm text-dim ">Convert JSON to XML.</p>

      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <section className="p-5 rounded-2xl bg-surface-2 border border-hairline space-y-4">
          <h3 className="font-medium text-text ">Your JSON content</h3>
          <textarea
            value={jsonInput}
        ...
        <section className="p-5 rounded-2xl bg-surface-2 border border-hairline space-y-4">
          <h3 className="font-medium text-text ">Converted XML</h3>
          <textarea
            value={xmlOutput}
            readOnly
          ...
          <div className="flex gap-2">
            <button onClick={() => copy(xmlOutput)} className="px-4 h-10 rounded-xl border border-hairline hover:bg-surface-2 dark:hover:bg-surface-2 transition-smooth text-text">Copy XML</button>
          </div>
        </section>
      </div>
    </div>
