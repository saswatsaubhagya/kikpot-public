...
export default function JsonToYaml() {
  ...
  return (
    <div className="space-y-6">
      <p className="text-sm text-dim ">Simply convert JSON to YAML with this online live converter.</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <section className="p-5 rounded-2xl bg-surface-2 border border-hairline space-y-4">
          <h3 className="font-medium text-text ">YAML from your JSON</h3>
          <textarea
            value={yamlOutput}
            readOnly
          ...
          />
          <div>
            <button onClick={() => copy(yamlOutput)} className="px-4 h-10 rounded-xl border border-hairline hover:bg-surface-2 dark:hover:bg-surface-2 transition-smooth text-text">Copy YAML to clipboard</button>
          </div>
        </section>
      </div>
    </div>
